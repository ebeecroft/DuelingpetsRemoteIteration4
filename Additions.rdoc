...
== Iteration 4
== User Content
* Accounttype: The difficulty in keeping accounttype is it places strain on available points and emeralds stored in storage accounts. When points and emeralds are placed in storage that means they are not out there buying things and reduce the amount of capital flowing through the economy. This also has a negative affect on the Dragonhoard which is a fundamental part of balancing the economy. The Central Bank uses the points it gets from taxes to create emeralds for the user to buy. So what this means is the Central Bank would be unable to purchase pets and items from the player or to perform upgrades to its Warehouse. The Central Bank is highly reliant on users making purchases as it doesn't create points on its own. Since email has been changed to be one email per account for recovery features, it made no sense to keep this feature floating around.

== Changes
== Iteration 4
== User Content
* Gviewer: Renamed blogviewers to gviewers to provide a generic viewer object that can be used with any table.
...
* Warehouse: A place for users to purchase items and pets that the Dragonhoard owns. In order to refill its inventory the warehouse will purchase the items and pets from the artist for a modest fee. Any points and emeralds gained from here can then be transfered back to the central bank. Witemshelves is an item storage system for the warehouse, and can hold up to a max of 30 items per shelf. Wpetdens is a pet storage system for the warehouse, and can hold up to a max of 16 pets per den. Each of these features can raise or lower the tax on each individual item or pet to adjust the price of the object.
...
== User Information
* Regtoken: Allows a user to register for an account if they are underage and parental consent has been received.
